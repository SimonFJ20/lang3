async function main() {
    ...
    console.log(
        tyToString(
            ch.exprTy(
                (((ast[1].kind as StmtKind & { tag: "fn" }).body[0].kind as
                    & StmtKind
                    & { tag: "let" }).expr.kind as ExprKind & { tag: "call" })
                    .expr,
            ),
        ),
    );
...
export type Ty =
    | { tag: "error" }
    | { tag: "int" }
    | { tag: "fn"; stmt: Stmt; params: Ty[]; returnTy: Ty };
...
export function tyToString(ty: Ty): string {
    switch (ty.tag) {
        ...
        case "int":
            return `int`;
        case "fn": {
            const k = ty.stmt.kind as StmtKind & { tag: "fn" };
...
export class Resols {
    ...
    public toString(): string {
        return JSON.stringify(
            {
                stmtResols: Object.fromEntries(
                    this.stmtResols.entries().map((
                        [k, v],
                    ) => [k, resolveToString(v)]),
                ),
                exprResols: Object.fromEntries(
                    this.exprResols.entries().map((
                        [k, v],
                    ) => [k, resolveToString(v)]),
                ),
            },
...
export class Resolver {
    ...
    private resolveBlock(stmts: Stmt[]) {
        const outerSyms = this.syms;
...
export type StmtKind =
    | { tag: "error" }
    | { tag: "fn"; ident: string; params: string[]; body: Stmt[] }
    | { tag: "let"; ident: string; expr: Expr }
    | { tag: "loop"; body: Stmt[] }
    | { tag: "if"; expr: Expr; truthy: Stmt[]; falsy?: Stmt[] }
...
export function lex(text: string): Tok[] {
    const ops = "(){}+*=,;";
    const kws = ["let", "fn", "return", "if", "loop", "break"];
